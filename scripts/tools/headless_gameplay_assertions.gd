...
func _process(delta: float) -> bool:
	...
	assert(FileAccess.file_exists("res://assets/art/generated/ui/hud_panel_left.png"), "Split pixel UI panel should exist")
	assert(hud.stats_art == null and hud.right_art == null, "HUD should not show the old full-sheet translucent UI image")
	assert(hud.stats_panel == null and hud.right_panel == null, "Top red/blue HUD panel frames should be removed")
	...
	assert(game.object_pools.has("effect"), "Combat effects should be pooled")
	assert(game.object_pools.has("weapon_zone"), "Weapon zones should be pooled")
	assert(game.object_pools.has("ui_burst"), "Jackpot rainbow burst particles should be pooled")
	for pool_id in game.object_pools.keys():
		assert((game.object_pools[pool_id] as Array).size() >= 20, "Object pool %s should be prewarmed with at least 20 nodes" % pool_id)
	...
	assert(game.object_pools.has("pickup"), "Pickups, including chests, should be pooled")
	...
	assert(wave_director.max_alive_enemies >= 220, "Alive enemy cap should rise for denser hordes")
	...
	assert(wave_two_enemy.max_health / wave_one_enemy.max_health >= 1.29, "Enemy health should grow sharply enough to resist runaway player scaling")
	...
	assert(bool(batched_enemy.get("batched_visual_enabled")), "Normal enemies should use the GPU batch renderer")
	assert(game.enemy_batch_renderer.get_child(0).multimesh.visible_instance_count >= 1, "GPU enemy batch should expose visible instances")
	...
	assert(FileAccess.file_exists("res://assets/art/generated/ui/slot_frame.png"), "Slot machine should use split pixel UI frame art")
	hud.hide_level_up()
	var elite: Node2D = game.EnemyScene.instantiate()
	game.add_child(elite)
	elite.configure("elite", 1)
	assert(is_equal_approx(elite.elite_scale_multiplier, 2.0), "Elite scale multiplier should shrink from 5x to 2x")
	assert(is_equal_approx(elite.radius, 28.0 * 1.5 * 2.0), "Elite collision radius should return to the 720x1280 scale")
	assert(elite.max_health >= 170.0 * 100.0, "Elite health should be 100x")
	assert(elite.elite_variant != "", "Elite enemies should roll visible gameplay variants")
	assert(elite.animation_state == "idle", "Enemy animation state should initialize as idle")
	elite.target = player
	elite._physics_process(0.016)
	assert(elite.animation_state == "run", "Enemy animation state should switch to run when chasing")
	elite.take_damage(1.0)
	assert(elite.animation_state == "hit", "Enemy animation state should switch to hit when damaged")
	elite._set_animation_state("attack", true)
	assert(elite.animation_state == "attack", "Enemy animation state should support attack animations")
	elite.queue_free()
	var stats_color: Color = hud.stats.get_theme_color("font_color")
	assert((stats_color.r + stats_color.g + stats_color.b) / 3.0 >= 0.88, "HUD text should be bright on dark backgrounds")
	assert(hud.performance_label != null and hud.performance_label.visible, "Performance HUD should be visible")
	assert(hud.performance_label.horizontal_alignment == HORIZONTAL_ALIGNMENT_RIGHT, "Performance HUD should sit in the lower-right corner")
	assert(hud.performance_label.text.contains("FPS"), "Performance HUD should include FPS")
	assert(hud.health_bar.offset_bottom <= hud.level_label.offset_top + 8.0, "Health bar should sit above the bottom level text")
	assert(hud.health_label.text.contains("/"), "Health label should show current/max health")
	assert(hud.weapons_label.autowrap_mode != TextServer.AUTOWRAP_OFF, "Weapon summary should wrap instead of piling up")
	assert(hud.relics_label.autowrap_mode != TextServer.AUTOWRAP_OFF, "Relic summary should wrap instead of piling up")
	assert(wave_director.wave_duration >= 32.0, "Wave duration should be doubled")
	var old_wave: int = int(wave_director.wave)
	wave_director.time_left = 0.0
	wave_director.tick(0.2, 1)
	assert(wave_director.wave == old_wave, "Wave should wait for all current enemies to die before advancing")
	wave_director.tick(0.2, 0)
	assert(wave_director.wave == old_wave + 1, "Wave should advance after time ends and all enemies are cleared")
	var knockback_enemy: Node2D = game.EnemyScene.instantiate()
	game.add_child(knockback_enemy)
	knockback_enemy.configure("chaser", 1)
	knockback_enemy.world_size = Vector2(300, 300)
	knockback_enemy.target = player
	knockback_enemy.global_position = Vector2(294, 150)
	knockback_enemy.apply_knockback(Vector2.RIGHT, 1200.0)
	knockback_enemy._physics_process(0.2)
	assert(knockback_enemy.global_position.x <= knockback_enemy.world_size.x - knockback_enemy.radius, "Knockback should not push enemies outside the map")
	knockback_enemy.apply_slow(0.5, 0.4)
	assert(knockback_enemy.modulate.b > knockback_enemy.modulate.r, "Slowed enemies should visibly turn blue")
	knockback_enemy.queue_free()
	wave_director.wave = 20
